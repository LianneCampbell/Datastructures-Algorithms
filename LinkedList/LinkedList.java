...
public class LinkedList {
    ...
    public void deleteNode(int location) {
        if (head == null) {
            ...
            return;
        } else if (location < 0 || location >= size) { // Handle invalid location
            System.out.println("Invalid location. Node cannot be deleted.");
            return;
        } else if (location == 0) { // Delete the first node
            head = head.next;
            size--;
            if (size == 0) { // If the list becomes empty, reset tail
                tail = null;
            }
            ...
            tail = tempNode;
            size--;
        } else { // Delete a node at any given location
            Node tempNode = head;
